...
export default function DashboardLayout({ children, activePage, currentUser }: DashboardLayoutProps) {
  ...
  const userType = effectiveUser.user_metadata?.user_type || 'client';

  const getMenuItems = () => {
    switch (userType) {
      case 'producer':
        return [
          { id: 'dashboard', label: 'Dashboard', icon: <BarChart3 className="h-4 w-4" />, href: '/dashboard' },
          { id: 'products', label: 'Mes Produits', icon: <Package className="h-4 w-4" />, href: '/dashboard/products' },
          { id: 'orders', label: 'Commandes Reçues', icon: <ShoppingCart className="h-4 w-4" />, href: '/dashboard/orders' },
          { id: 'propositions', label: 'Propositions Reçues', icon: <FileText className="h-4 w-4" />, href: '/dashboard/propositions' },
          { id: 'requests', label: 'Demandes Disponibles', icon: <Search className="h-4 w-4" />, href: '/dashboard/requests' },
          { id: 'notifications', label: 'Notifications', icon: <Bell className="h-4 w-4" />, href: '/dashboard/notifications' },
          { id: 'stats', label: 'Statistiques', icon: <TrendingUp className="h-4 w-4" />, href: '/dashboard/stats' },
          { id: 'location', label: 'Géolocalisation', icon: <MapPin className="h-4 w-4" />, href: '/dashboard/location' },
          { id: 'account', label: 'Mon Profil', icon: <UserIcon className="h-4 w-4" />, href: '/dashboard/account' },
      ...
      case 'client':
        return [
          { id: 'dashboard', label: 'Accueil', icon: <Home className="h-4 w-4" />, href: '/dashboard/client' },
          { id: 'orders', label: 'Mes Commandes', icon: <Package className="h-4 w-4" />, href: '/dashboard/client/orders' },
          { id: 'requests', label: 'Demandes de Livraison', icon: <FileText className="h-4 w-4" />, href: '/dashboard/client/requests' },
          { id: 'favorites', label: 'Mes Favoris', icon: <Heart className="h-4 w-4" />, href: '/dashboard/client/favorites' },
          { id: 'reviews', label: 'Historique & Évaluations', icon: <Star className="h-4 w-4" />, href: '/dashboard/client/reviews' },
          { id: 'profile', label: 'Mon Profil', icon: <UserIcon className="h-4 w-4" />, href: '/dashboard/client/profile' },
          { id: 'logout', label: 'Déconnexion', icon: <LogOut className="h-4 w-4" />, href: '#' }
        ];
      default:
        return [];
            ...
            <Card className="p-6">
              ...
              <div className="text-center mb-6">
                <div className="w-16 h-16 rounded-full overflow-hidden mx-auto mb-3">
                  <img 
                    src={effectiveUser.user_metadata?.avatar_url || "https://images.pexels.com/photos/1239291/pexels-photo-1239291.jpeg?auto=compress&cs=tinysrgb&w=100"} 
                    alt={effectiveUser.user_metadata?.name || effectiveUser.email}
                    className="w-full h-full object-cover"
                  />
                </div>
                <h3 className="font-semibold text-gray-900">{effectiveUser.user_metadata?.name || effectiveUser.email}</h3>
                <p className="text-sm text-gray-500">{effectiveUser.email}</p>
                <p className={`text-xs ${getProfileColor(userType)} capitalize`}>
                  {getProfileLabel(userType)}
                </p>
              </div>
                  ...
                  item.id === 'logout' ? (
                    ...
                    <Link
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-colors ${
                        activePage === item.id 
                          ? `${userType === 'producer' ? 'bg-matix-green-pale text-matix-green-dark' : 
                              userType === 'distributor' ? 'bg-blue-100 text-blue-700' : 
