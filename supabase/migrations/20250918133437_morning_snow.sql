...
DROP POLICY IF EXISTS "Allow public read for verified business profiles" ON public.users;
...
-- Synchroniser les données existantes avec auth.users si possible
DO $$
BEGIN
    -- Mettre à jour les utilisateurs existants avec les données d'auth.users
    UPDATE public.users 
    SET 
    ...
    RAISE NOTICE 'Données utilisateur synchronisées avec auth.users';
EXCEPTION
    WHEN others THEN
        RAISE WARNING 'Erreur lors de la synchronisation: %', SQLERRM;
END $$;
...
SELECT 
    'Structure de la table users' as info,
    column_name,
    data_type,
    is_nullable,
    column_default
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🔗 URLs à configurer dans Supabase Dashboard :';
