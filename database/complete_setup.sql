-- Script complet pour réinitialiser et configurer la base de données MATIX
...
-- Fonction pour rechercher des produits à proximité
...
BEGIN
    ...
    SELECT 
        ...
        p.description,
        p.price,
        calculate_distance(search_lat, search_lng, ST_Y(p.location_coordinates), ST_X(p.location_coordinates)) as distance_km
    FROM products p
    WHERE p.location_coordinates IS NOT NULL
    AND calculate_distance(search_lat, search_lng, ST_Y(p.location_coordinates), ST_X(p.location_coordinates)) <= search_radius
    ORDER BY distance_km;
END;
$$ LANGUAGE plpgsql;

-- Fonction pour obtenir les demandes dans une zone
...
BEGIN
    ...
    SELECT 
        ...
        dr.quantity,
        calculate_distance(search_lat, search_lng, ST_Y(dr.delivery_coordinates), ST_X(dr.delivery_coordinates)) as distance_km
    FROM delivery_requests dr
    WHERE dr.delivery_coordinates IS NOT NULL
    AND dr.status = 'pending'
    AND calculate_distance(search_lat, search_lng, ST_Y(dr.delivery_coordinates), ST_X(dr.delivery_coordinates)) <= search_radius
    ORDER BY distance_km;
END;
$$ LANGUAGE plpgsql;
...
CREATE POLICY "Distributors can view requests in their zone" ON delivery_requests FOR SELECT USING (
    EXISTS (
        ...
        AND calculate_distance(
            ST_Y(users.coordinates), ST_X(users.coordinates),
            ST_Y(delivery_requests.delivery_coordinates), ST_X(delivery_requests.delivery_coordinates)
        ) <= users.radius_km
    )
);
